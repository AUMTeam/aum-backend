...
function getUserData(DatabaseWrapper $db, string $token, array $data = []) : array {
    ...
    if(isset($data['user_id']))
        ...
        $user_data = $db->query("SELECT user_id FROM users_v2_tokens WHERE token = '$token'");  //FIXME: Duplicated
    ...
    $user_data = $db->query("SELECT user_id, name, area_id, email, role_id FROM users_v2 WHERE user_id = {$user_data[0]['user_id']}");
