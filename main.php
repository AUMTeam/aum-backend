...
//POST Method
    ...
    try {
        ...
        if(empty($request))
            throw new InvalidRequestException("Request is not a JSON", "ERROR_REQUEST_NOT_JSON");
        ...
        //Ignoring token check only when login is called
        if(!($module == "auth" && $action == "login")) {
            ...
            if($token == null)
                ...
                getTokenExpire();   //Check if the token is valid (we aren't interested in the return value)
    ...
    } catch (Exception $exception) {    //Unmanaged exceptions
        //Fatal error
        $response = [
            'response_data' => [],
            'status_code' => 500,
