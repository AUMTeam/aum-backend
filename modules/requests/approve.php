...
$exec = function (array $data, array $data_init) : array {
    ...
    $query = $db->query("SELECT is_approved FROM requests WHERE request_id={$data['request_id']}");
    if (count($query) == 0)
    ...
    $db->query("UPDATE requests SET is_approved = {$data['approve_flag']} WHERE request_id={$data['request_id']}");
