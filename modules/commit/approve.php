...
$exec = function (array $data, array $data_init) : array {
    ...
    $query = $db->query("SELECT is_approved FROM commits_v2 WHERE commit_id={$data['id']}");
    if (count($query) == 0)
    ...
    $db->query("UPDATE commits_v2 SET is_approved = {$data['approve_flag']}, approvation_comment = {$approvation_comment}, approver_user_id={$user_id}  WHERE commit_id={$data['id']}");
